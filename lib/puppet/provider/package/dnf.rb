Puppet::Type.type(:package).provide :dnf, :parent => :rpm, :source => :rpm do
  ...
  def self.prefetch(packages)
    raise Puppet::Error, "The dnf provider can only be used as root" if Process.euid != 0
    super
  end
  ...
  def self.check_updates(enablerepo, disablerepo, disableexcludes)
    ...
    if output.exitstatus == 100
      ...
      self.debug "dnf check-update exited with 0; no package updates available."
    else
      self.warn "Could not check for updates, 'dnf check-update' exited with #{output.exitstatus}"
    end
    updates
  end
  ...
  def self.parse_updates(str)
    ...
    body.split.each_slice(3) do |tuple|
      ...
      # version since dnf considers those as mostly interchangeable.
  ...
  def install
    ...
    unless @resource.allow_virtual?
      dnf *['-d', '0', '-e', '0', '-y', install_options, :list, wanted].compact
    end
    ...
    when true, false, Symbol
      ...
      wanted += "-#{should}"
      is = self.query
      if is && dnf_compareEVR(dnf_parse_evr(should), dnf_parse_evr(is[:ensure])) < 0
    ...
    if should
      ...
      # and dnf updated us to a version other than @param_hash[:ensure] ?
      vercmp_result = dnf_compareEVR(dnf_parse_evr(should), dnf_parse_evr(is[:ensure]))
  ...
  def latest
    ...
    unless upd.nil?
      ...
      # dnf didn't find updates, pretend the current
  ...
  def update
    # Install in dnf can be used for update, too
    self.install
  end

  def purge
    dnf "-y", :erase, @resource[:name]
  end

  # parse a dnf "version" specification
  # this re-implements dnf's
  ...
  # how dnf compares two package versions:
  ...
  def dnf_compareEVR(should_hash, is_hash)
  ...
  # found in python/header-py.c, as used by dnf.
